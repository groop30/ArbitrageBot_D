    ...
    if bb_down_1h > high_1h:
        ...
        if open_1h > close_1h:
            ...
            if diff_body_per < 0.5:
                # если тело свечи меньше полпроцента - то за стоп берем лой
                stop_level = low_1h
            else:
                stop_level = open_1h
        ...
        if open_1h < close_1h:
            ...
            diff_body_per = diff_body / open_1h * 100
            if diff_body_per < 0.5:
                # если тело свечи меньше полпроцента - то за стоп берем high
                stop_level = high_1h
            else:
                stop_level = open_1h
...
# ##############################################
# блок управения лимитными ордерами
    ...
    for index in range(len(check_df)):
        ...
        if len(opened_df) > 0:
            ...
            if len(opened_df) == 1:
                ...
                # или считать в целевых расстояниях. (ушла против на 1(? 2? 3?) расстояние до цели- усредняем
                ...
                if strategy == 'zscore':
                    ...
                    if going_to == 'UP':
                        ...
                        if z_condition_1 & z_condition_2:
                            ...
                            open_pair_position(coin1, coin2, "UP", pos_size, lookback, stop, True)
                    elif going_to == 'DOWN':
                        ...
                        if z_condition_1 & z_condition_2:
                            ...
                            open_pair_position(coin1, coin2, "DOWN", pos_size, lookback, stop, True)
                    continue
                elif strategy == 'bb1' or strategy == 'bb2':
                    # на обратном пересечении (внутрь), если от первого входа цена ушла > 1%
                    if going_to == 'UP':
                        price_diff = (op_price - l_price) / op_price * 100
                        if l_price > l_bb_down and pre_high < pre_bb_down and price_diff > 1:
                            print(f'{coin1}/{coin2}-открываем позицию bb2,UP, цена={l_price}, в {l_time}')
                            open_pair_position(coin1, coin2, "UP", pos_size, lookback, stop, True)
                    elif going_to == 'DOWN':
                        price_diff = (l_price - op_price) / op_price * 100
                        if l_price < l_bb_up and pre_low > pre_bb_up and price_diff > 1:
...
def open_pair_position(coin1, coin2, going_to, amount, lookback, stop=0.0, limit=True):
    ...
    if limit:
        ...
        try:
            ...
            print(f'ошибка рассчета цены сделки - {error}')
    ...
    # Убрать после теста
    # close_pair_position(coin1_id, coin2_id, coin1, coin2, pos_size1, pos_size2, price, new_row)
...
def open_file_positions():
    ...
    filepath_open = r'.\reports\bin_to_open.csv'
    ...
    if path.exists(filepath_open):
        ...
        for index in range(len(open_df)):
            ...
            open_pair_position(coin1, coin2, going_to, amount, lookback, True)
...
def check_for_close():
    ...
    for index in range(len(close_df)):
        ...
        coin1_id, coin2_id, coin1, coin2, going_to, stop,  size1, size2, lookback = (
            close_df[c].to_numpy()[index]
            for c in close_df
            if c in ["coin1_id", "coin2_id", "coin1", "coin2", "going_to", "stop", "size1", "size2", "lookback"]
        )
        df = calc_last_data(coin1, coin2, lookback, tf_5m)
...
# schedule.every(20).seconds.do(check_conditions)
