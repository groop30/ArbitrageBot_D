...
exception_list = ['BTSUSDT', 'SCUSDT', 'TLMUSDT', 'BTCSTUSDT', 'FTTUSDT', 'XEMUSDT', 'XLMUSDT']
...
def get_statistics(coin1, coin2, coin1_hist, coin2_hist, use_filter=True):
    ...
    if use_filter:
        ...
        if (stat_coin3 < 0.005) and (stat_coin1 > 0.05) and (stat_coin2 > 0.05) \
                and (coint_coeff < 0.03) and (corr_coeff > 0.8):
            add_row = True
    else:
        add_row = True
...
def get_screening_result(full_control):
    ...
    start_time = datetime.datetime.now().timestamp() - 1000 * tf_5m
    for future in all_futures:
...
def summarize_actual_pairs_stability(go_next=False):
    ...
    if go_next:
        get_historical_strategy_result(full_result=True, best_only=True, file=1, days=14)
...
def get_historical_strategy_result(full_result=True, best_only=True, file=2, days=14, cut_df=500):
    ...
    if file == 2:
        ...
        get_historical_strategy_result(full_result=True, best_only=True, file=4, days=14, cut_df=50)
    elif file == 1:
...
# get_historical_strategy_result(full_result=True, best_only=True, file=1, days=14) # полный расчет, за 7 дней
...
# get_screening_result(False)
check_parameters_stability(window=500, steps=6, shift=72, file=3, go_next=False)
